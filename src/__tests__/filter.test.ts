...
describe('filterReposByOrganization', () => {
  ...
  it('should filter to single organization', () => {
    ...
    expect(result.every((r: RepoData) => r.repo.startsWith('developmentseed/'))).toBe(true);
  });
...
describe('filterLinksByRepos', () => {
  it('should filter links to only visible repos', () => {
    ...
    expect(result).toHaveLength(2);
    expect(result.every((l: LinkData) => l.repo === 'developmentseed/titiler')).toBe(true);
  });
...
describe('filterLinksByContributors', () => {
  it('should filter links to only visible contributors', () => {
    ...
    expect(result).toHaveLength(2);
    expect(result.every((l: LinkData) => l.author_name === 'Alice')).toBe(true);
  });
});
...
describe('filterContributorsByLinks', () => {
  it('should filter contributors to those with visible links', () => {
    ...
    expect(result).toHaveLength(2);
    expect(result.map((c: ContributorData) => c.author_name)).toEqual(['Alice', 'Bob']);
  });
});
...
describe('filterReposByStars', () => {
  it('should filter repos below the star threshold', () => {
    ...
    expect(result).toHaveLength(3);
    expect(result.every((r: RepoData) => +(r.repo_stars ?? 0) >= 100)).toBe(true);
  });
...
describe('filterReposByForks', () => {
  it('should filter repos below the fork threshold', () => {
    ...
    expect(result).toHaveLength(3);
    expect(result.every((r: RepoData) => +(r.repo_forks ?? 0) >= 100)).toBe(true);
  });
...
describe('applyFilters', () => {
  ...
  it('should filter by organization correctly', () => {
    ...
    expect(result.contributors.map((c: ContributorData) => c.author_name).sort()).toEqual(['Alice', 'Bob']);
  ...
  it('should return deep clones to prevent mutation', () => {
    ...
    expect((originalData.repos![0] as RepoData & { stars: number }).stars).toBe(100);
    expect((originalData.contributors![0] as ContributorData & { commits: number }).commits).toBe(100);
  ...
  it('should correctly chain filters (repos -> links -> contributors -> links)', () => {
    ...
    expect(result.contributors).toHaveLength(2);
    expect(result.contributors.map((c: ContributorData) => c.author_name).sort()).toEqual(['Bob', 'Charlie']);
...
describe('createFilterManager', () => {
  ...
  it('should call onChange callback when filters change', () => {
    let callCount = 0;
    let lastFilters: ReturnType<ReturnType<typeof createFilterManager>['getFilters']> | null = null;
  ...
  it('should ignore invalid metric names', () => {
    const manager = createFilterManager();
    manager.setMetricFilter('invalidMetric' as 'starsMin', 100);
